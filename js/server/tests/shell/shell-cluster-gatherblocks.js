...
var debug = true; //FIXME - must be false before merge into devel
    ...
    testRuleBasics : function () {
      if(enabled.basics){
        ...
        tests.forEach(t => {
            ...
            // check that the plan contains the expected amount of gahterNodes
            ...
            // assert that the sortmode set in the gathermode is correct corresponding
            // to the amount of shards
            ...
            // chech that the returned result is sorted when requeted
            if(t.sorted) {
              var old = -1;
              rv.forEach(d => {
                assertMessage = "\n##### test: " + t.test + " - " + old + " -> " + d + " #####\n";
